...
app.post('/api/admin/settings/email/test', async (c) => {
  try {
    const body = await c.req.json().catch(() => ({}));
    const { sendTo, smtpUser, emailFrom, emailFromName, smtpHost, smtpPort } = body;
    const recipient = sendTo || smtpUser || 'workbridge.anandhu@gmail.com';
    ...
    if (!result.success) {
      return c.json({
        success: false,
        message: result.error || 'Failed to deliver test email via SMTP relay.',
      }, 500);
    }
    ...
    return c.json({ success: false, message: err.message || 'SMTP test error' }, 500);
  }
});
...
app.post('/api/calls/rooms/:roomName/join', async (c) => {
  try {
    ...
    if (!resolvedName || resolvedName === 'Candidate' || resolvedName === 'Interviewer' || resolvedName === 'Participant') {
      try {
        const interviewRow: any = await c.env.DB.prepare(`
          SELECT jsp.fullName FROM "Interview" i
          JOIN "Application" a ON a.id = i.applicationId
          JOIN "JobSeekerProfile" jsp ON jsp.id = a.jobSeekerProfileId
          WHERE i.id = ?
        `).bind(roomName).first();
