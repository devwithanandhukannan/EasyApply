...
export const updateOfferTemplate = async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = req.company?.companyId;
...
const generateOfferPDF = async (offer, content) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (profile.phone)
            doc.text(`Phone: ${profile.phone}`, L, 177, { lineBreak: false });
        doc.restore();
        // ── GREETING ───────────────────────────────────────────────────────────────
        doc.save();
        ...
        doc.font('Helvetica-Oblique').fontSize(8.5).fillColor('#4b5563')
            .text('To indicate formal confirmation of acceptance, please digitally execute your authorization signature below.', L, doc.y + 8, { width: PW });
        // ── SIGNATURE BLOCK ────────────────────────────────────────────────────────
        ...
        if (offer.companySignature?.signature) {
            try {
                ...
                if (compImg)
                    doc.image(compImg, L, finalSigY + 14, { width: 100, height: 28, fit: [100, 28] });
            }
            catch (e) {
        ...
        doc.font('Helvetica').fontSize(7.5).fillColor('#6b7280').text(offer.companySignature ? `Verified: ${new Date(offer.companySignature.signedAt).toLocaleDateString()}` : 'Awaiting Corporate Stamp', L, finalSigY + 58, { lineBreak: false });
        doc.restore();
        // Candidate sig
        ...
        if (offer.candidateSignature?.signature) {
            try {
                ...
                if (candImg)
                    doc.image(candImg, rightSignX, finalSigY + 14, { width: 100, height: 28, fit: [100, 28] });
            }
            catch (e) {
        ...
        doc.font('Helvetica').fontSize(7.5).fillColor('#6b7280').text(offer.candidateSignature ? `Digitally Signed: ${new Date(offer.candidateSignature.signedAt).toLocaleDateString()}` : 'Awaiting Signatures', rightSignX, finalSigY + 58, { lineBreak: false });
        doc.restore();
        // ── FOOTER ─────────────────────────────────────────────────────────────────
...
const parseSignatureImage = (sigData) => {
...
export const updateOfferLetter = async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = req.company?.companyId;
        const { position, department, salary, currency, startDate, location, employmentType, content } = req.body;
...
export const createOfferLetter = async (req, res) => {
    ...
    try {
        ...
        const { applicationId, templateId, position, department, salary, currency, startDate, location, employmentType, customContent } = req.body;
...
// ─── DIGITAL SIGNATURE ─────────────────────────────────────────────
export const signOfferLetter = async (req, res) => {
    try {
        const { id } = req.params;
...
export const sendOfferLetter = async (req, res) => {
    try {
        ...
        if (channels.includes('email')) {
            await sendOfferLetterEmail(offer.application.jobSeekerProfile.email, offer.application.jobSeekerProfile.fullName, offer.position, offer.application.jobPosting.company.name, offer.application.jobPosting.company.logoUrl, offer.id, pdfPath);
...
// ─── EMAIL TRACKING ────────────────────────────────────────────────
export const trackOfferEmail = async (req, res) => {
    try {
        const { id } = req.params;
...
export const respondToOffer = async (req, res) => {
    try {
        ...
        if (companyEmail) {
            await sendOfferResponseNotification(companyEmail, offer.application.jobSeekerProfile.fullName, offer.position, response, negotiationNote);
        }
        return res.json({ success: true, data: updated });
    }
    catch (error) {
...
export const generateTemplateWithAI = async (req, res) => {
    try {
        ...
        const aiResult = await generateOfferLetterTemplate(name, description, companyName);
        return res.json({
            success: true,
            data: aiResult,
...
export const createOfferTemplate = async (req, res) => {
    try {
        const companyId = req.company?.companyId;
        const companyName = req.company?.name;
        if (!companyId) {
...
export const respondToNegotiation = async (req, res) => {
    try {
        ...
        if (action === 'accept_negotiation') {
            ...
            await sendOfferLetterEmail(offer.application.jobSeekerProfile.email, offer.application.jobSeekerProfile.fullName, offer.position, offer.application.jobPosting.company.name, offer.application.jobPosting.company.logoUrl, offer.id, '');
            return res.json({
                success: true,
                data: updated,
...
export const getSalaryComparison = async (req, res) => {
    try {
        ...
        const trackingMetrics = await aggregateSalaryBenchmarks(String(title), String(location), experience ? String(experience) : '1-3 Years', offeredSalary ? String(offeredSalary) : undefined);
        res.status(200).json({
            success: true,
