 ...
 */
export const getCrmCandidates = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (status) where.status = status as CandidateCrmStatus;
    if (isStarred === 'true') where.isStarred = true;
    if (priority) where.crmPriority = priority;
    if (source) where.source = source as CandidateSource;
 ...
 */
export const getCrmCandidateById = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const { id } = req.params;
 ...
 */
export const addCrmCandidate = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
 ...
 */
export const updateCrmCandidate = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const { id } = req.params;
    ...
    const updated = await prisma.companyCandidateProfile.update({
      where: { id },
      data: {
        ...(status !== undefined && { status: status as CandidateCrmStatus }),
        ...(crmNotes !== undefined && { crmNotes }),
        ...(crmPriority !== undefined && { crmPriority }),
        ...(tags !== undefined && { tags }),
        ...(isStarred !== undefined && { isStarred }),
        ...(ownerId !== undefined && { ownerId }),
        ...(source !== undefined && { source: source as CandidateSource }),
 ...
 */
export const removeCrmCandidate = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const { id } = req.params;
 ...
 */
export const logCrmInteraction = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const userId = req.user?.userId;
    const { id } = req.params;
 ...
 */
export const getTalentPools = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    if (!companyId) return res.status(401).json({ success: false, message: 'Unauthorized' });
 ...
 */
export const createTalentPool = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    if (!companyId) return res.status(401).json({ success: false, message: 'Unauthorized' });
 ...
 */
export const updateTalentPool = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const { id } = req.params;
    if (!companyId) return res.status(401).json({ success: false, message: 'Unauthorized' });
 ...
 */
export const deleteTalentPool = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const { id } = req.params;
    if (!companyId) return res.status(401).json({ success: false, message: 'Unauthorized' });
 ...
 */
export const getTalentPoolMembers = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const { id } = req.params;
    if (!companyId) return res.status(401).json({ success: false, message: 'Unauthorized' });
 ...
 */
export const addTalentPoolMembers = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
    const userId = req.user?.userId;
    const { id } = req.params;
    ...
    await prisma.talentPoolMember.createMany({
      data: (jobSeekerProfileIds as string[]).map((profileId) => ({
        talentPoolId: id,
 ...
 */
export const removeTalentPoolMember = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.company?.companyId;
