    ...
    createSpotJob: async (req, res) => {
        try {
            ...
            totalCandidatesInDB.forEach(candidate => {
                ...
                if (isOpen) {
                    if (normalizedRequiredSkills.length === 0) {
                        ...
                        const exactMatches = normalizedRequiredSkills.filter(reqSkill => candidateSkillsArray.includes(reqSkill));
                        if (exactMatches.length > 0) {
                            ...
                            const hasPartial = normalizedRequiredSkills.some(reqSkill => candidateSkillsArray.some(candSkill => candSkill.includes(reqSkill) || reqSkill.includes(candSkill)));
                            if (hasPartial) {
            ...
            if (eligibleCandidates.length > 0) {
                ...
                const notificationPromises = eligibleCandidates.map(candidate => NotificationService.sendToUser(candidate.userId, `🔥 Immediate Spot Gig Match!`, `Hi ${candidate.fullName}, a spot job matching your profile has just opened up: "${title}". Rate: ${rate} ${currency}/${rateType}`, `http://localhost:3000/dashboard/spot-jobs`).catch((err) => {
...
//# sourceMappingURL=spotJob.controller.js.map