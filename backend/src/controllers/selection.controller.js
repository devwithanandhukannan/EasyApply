...
export const bulkUpdateApplicationStatus = async (req, res) => {
    try {
        ...
        await prisma.$transaction([
            ...
            ...applications.map((app) => prisma.applicationHistory.create({
                data: {
...
export const bulkMoveApplications = async (req, res) => {
    try {
        const companyId = req.company?.companyId;
        ...
        const { applicationIds, targetStatus, notes } = req.body;
        if (!applicationIds || applicationIds.length === 0) {
...
export const bulkStarApplications = async (req, res) => {
    try {
        const companyId = req.company?.companyId;
        const userId = req.user?.userId;
        if (!companyId || !userId) {
...
// ─── SET PRIORITY ──────────────────────────────────────────────────
export const bulkSetPriority = async (req, res) => {
    try {
        const companyId = req.company?.companyId;
        const userId = req.user?.userId;
        if (!companyId || !userId) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const activities = applicationIds.map((appId) => ({
                applicationId: appId,
                activityType: priority ? 'PRIORITY_SET' : 'PRIORITY_CLEARED',
                performedBy: userId,
                metadata: { priority }
            }));
            await tx.applicationActivity.createMany({
...
export const getFilteredApplications = async (req, res) => {
    try {
        ...
        const { status, starred, priority, tags, search, limit = '20', page = '1' } = req.query;
        const where = {
            jobPostingId,
            jobPosting: { companyId }
        };
        ...
        if (search) {
            where.OR = [
                { jobSeekerProfile: { fullName: { contains: search, mode: 'insensitive' } } },
                { jobSeekerProfile: { email: { contains: search, mode: 'insensitive' } } }
            ];
        }
