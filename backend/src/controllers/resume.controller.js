...
const readContent = (resume) => resume.content ?? {};
const readAI = (resume) => resume.aiSuggestions ?? {};
const pushVersion = (contentData, label) => {
...
const compileResumeText = (data) => {
    ...
    if (data.contact) {
        const parts = [];
        if (data.contact.location)
            parts.push(data.contact.location);
        if (data.contact.phone)
            parts.push(data.contact.phone);
        if (data.contact.email)
            parts.push(data.contact.email);
        if (data.contact.links)
            parts.push(...data.contact.links);
        text += parts.join(' | ') + '\n';
    }
    if (data.summary) {
...
export const downloadUploadedPDF = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.userId;
        const profileId = await getProfileId(userId);
...
export const convertResumeToHTML = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.userId;
        const profileId = await getProfileId(userId);
...
export const getKeywordSuggestions = async (req, res) => {
    try {
        ...
        const contentData = readContent(resume);
        if (!contentData.htmlContent)
            return res.status(422).json({ success: false, message: 'Open in editor first' });
...
export const updateResume = async (req, res) => {
    try {
        ...
        if (versionLabel)
            pushVersion(contentData, versionLabel);
        if (htmlContent !== undefined)
            contentData.htmlContent = htmlContent;
        if (margins)
            contentData.margins = margins;
        if (template)
...
export const scoreContentOnly = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.userId;
        const profileId = await getProfileId(userId);
        ...
        const contentData = readContent(resume);
        if (!contentData.htmlContent)
            return res.status(422).json({ success: false, message: 'No HTML content to score' });
...
export const getInlineSuggestions = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.userId;
        const profileId = await getProfileId(userId);
...
export const improveSelectedText = async (req, res) => {
    try {
...
export const generateRegionalCV = async (req, res) => {
    try {
        const userId = req.user.userId;
