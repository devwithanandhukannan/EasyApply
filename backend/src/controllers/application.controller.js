...
const ensureJobSeeker = async (userId) => {
...
export const getTimelineDashboardView = async (req, res) => {
    try {
        const userId = req.user.userId;
        const profileId = await getProfileId(userId);
        if (!profileId)
...
export const updateCandidatePrivateNotes = async (req, res) => {
    try {
        const { id } = req.params;
...
export const withdrawApplication = async (req, res) => {
    try {
        ...
        const hasActiveInterview = application.interviews.some(i => ['confirmed', 'in_progress', 'completed'].includes(i.status));
