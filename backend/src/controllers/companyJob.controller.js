...
export const createJob = async (req, res) => {
    try {
        const { title, department, jobType, locationType, location, experienceRequired, skills, description, salaryRange, deadline, openings, status } = req.body;
...
export const generateAIDescription = async (req, res) => {
    try {
        const { roughDescription, title, department, locationType, experienceRequired, skills, salaryRange } = req.body;
        if (!roughDescription) {
            return res.status(400).json({ success: false, message: 'Initial writing summary text payload required.' });
        }
        const cleanPolishedTemplate = await generateJobDescription(roughDescription, title, department, locationType, experienceRequired, skills, salaryRange);
        return res.status(200).json({
            success: true,
