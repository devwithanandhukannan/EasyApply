...
export const getCompanyInterviewsList = async (req, res) => {
    try {
...
export const confirmInterviewPresence = async (req, res) => {
    try {
        const { id } = req.params;
...
export const getInterviewFeedbacksList = async (req, res) => {
    try {
        const { interviewId } = req.params;
...
export const upsertInterviewFeedback = async (req, res) => {
    try {
        const { interviewId } = req.params;
        const { technicalRating, communicationRating, problemSolvingRating, verdict, notes } = req.body;
        const userId = req.user?.id || req.user?.userId;
...
export const getInterviewFeedbackByCandidate = async (req, res) => {
    try {
...
export const getApplicationDetailById = async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = req.company?.companyId;
        if (!companyId) {
...
export const scheduleBulkInterviews = async (req, res) => {
    try {
        const { jobPostingId, startTime, slotDuration, interviewFormat, interviewerIds, selectedApplicationIds, targetStatus } = req.body;
        ...
        await prisma.$transaction([
            ...
            ...selectedApplicationIds.map((applicationId) => prisma.applicationHistory.create({
                data: {
                    applicationId,
