...
const generateLiveKitToken = async (roomName, participantIdentity, participantName) => {
...
export const getCompanyToken = async (req, res) => {
    try {
...
export const getJobSeekerToken = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { id: interviewId } = req.params;
        if (!userId) {
