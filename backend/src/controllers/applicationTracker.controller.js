...
export const getApplicationsTracker = async (req, res) => {
    try {
        ...
        const mappedTrackerData = applications.map(app => {
            ...
            const hasInterviewsStarted = app.interviews.some(i => ['confirmed', 'in_progress', 'completed'].includes(i.status));
...
export const withdrawApplicationTracker = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { id: applicationId } = req.params;
        if (!userId) {
...
export const getSingleApplicationDetails = async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        const hasInterviewsStarted = application.interviews.some(i => ['confirmed', 'in_progress', 'completed'].includes(i.status));
...
//# sourceMappingURL=applicationTracker.controller.js.map