  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const app = await tx.application.update({
        where: { id: applicationId },
        data: { status: status as ApplicationStatus, lastActivityAt: new Date() },
...
export const searchCandidates = async (req: Request, res: Response) => {
  try {
    ...
    let candidatesList = Array.from(candidateMap.values()).filter((c) => {
      ...
      if (skills?.length) {
        const hasSkill = skills.some((s: string) => c.skills.some((cs: string) => cs.toLowerCase().includes(s.toLowerCase())));
    ...
    if (jobDescription?.trim().length > 0) {
      const candidatesForAI = candidatesList.map((c) => ({
        ...
        fullName: c.fullName,
        skills: c.skills,
        experience: c.experience.map((e: any) => ({ company: e.company, role: e.role, startYear: e.startYear, endYear: e.endYear })),
      ...
      candidatesList = candidatesList
        ...
        .sort((a, b) => {
          const aScore = rankings.find((r: any) => r.candidateId === a.profileId)?.score || 0;
          const bScore = rankings.find((r: any) => r.candidateId === b.profileId)?.score || 0;
          return bScore - aScore;
        });
    }

    const result = candidatesList.map((c) => {
      const rank = rankings.find((r: any) => r.candidateId === c.profileId);
      return {
